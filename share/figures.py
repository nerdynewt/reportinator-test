...
y_name_list=[]
y_list=[]
in_file = cache_dir+"/csvs/"+args.file
...
def plot(x_name, y_name_list, data, n, k):
    ...
    if "$" in y_name:
        ...
        y_new = y_new.replace("\\","")
        f.savefig(cache_dir+"/"+str(n)+y_new.split(" ")[0]+str(n)+".pdf", bbox_inches = 'tight')
    else:
        f.savefig(cache_dir+"/"+str(n)+y_name.split(" ")[0]+str(n)+".pdf", bbox_inches = 'tight')
    return cap
